...
>  * Clients will run client executable, with terminal prompts for user authentication upon connection. Upon connection, client messages will be broadcast and received by other clients. Client will disconnect upon specific user input format. the server will also terminate upon specific input format by a server admin.
...
>  * Discord is a common application that all of the team's members use, which is a part of why we are interested in how programming for a chat application works. 
